...
The complete OpenAPI 3.0 specification for all microservices is available in the [docs/openapi.yaml](docs/openapi.yaml) file.

### Quick Links
- **Live Swagger UI**: http://localhost:8080/swagger-ui.html (fully interactive after `docker-compose up`)
- **OpenAPI Spec File**: [docs/openapi.yaml](docs/openapi.yaml)
- **API Gateway**: http://localhost:8080/gateway

### API Overview

The API is organized into 6 service modules:

| Service | Base Path | Features |
|---------|-----------|----------|
| **Patients** | `/gateway/patients` | CRUD operations, search by name, blood type filtering |
| **Doctors** | `/gateway/doctors` | CRUD operations, filter by specialization, availability status |
| **Appointments** | `/gateway/appointments` | Book/update/cancel appointments, filter by patient/doctor/date, status management |
| **Pharmacy** | `/gateway/medicines` | Inventory management, stock updates, category filtering, search |
| **Billing** | `/gateway/invoices` | Invoice generation, payment tracking, status filtering |
| **Lab Tests** | `/gateway/labtests` | Order tests, submit results, filter by patient/doctor/status/category |

### Key Design Patterns
- **Standard REST**: GET, POST, PUT, DELETE, PATCH for all resources
- **Pagination**: Supported on list endpoints via query parameters
- **Status Enums**: Predefined statuses (e.g., PENDING, COMPLETED) prevent invalid states
- **Error Codes**: 200/201 for success, 400 for bad input, 404 for not found, 500 for server errors
